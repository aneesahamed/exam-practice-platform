...
# ── S3: SAM Deployment Artefacts ────────────────────────────────────────────

resource "aws_s3_bucket" "sam_deploy" {
  bucket = "${var.project_name}-${var.environment}-sam-deploy-${data.aws_caller_identity.current.account_id}"
}

resource "aws_s3_bucket_versioning" "sam_deploy" {
  bucket = aws_s3_bucket.sam_deploy.id
  versioning_configuration {
    status = "Enabled"
  }
}

resource "aws_s3_bucket_server_side_encryption_configuration" "sam_deploy" {
  bucket = aws_s3_bucket.sam_deploy.id
  rule {
    apply_server_side_encryption_by_default {
      sse_algorithm = "AES256"
...
resource "aws_s3_bucket_public_access_block" "sam_deploy" {
  bucket                  = aws_s3_bucket.sam_deploy.id
  block_public_acls       = true
  block_public_policy     = true
  ignore_public_acls      = true
